...
use package_discovery::{Module, Package};
...
impl ImportGraph {
    pub fn build(root_package_path: &Path) -> Result<ImportGraph> {
        let import_graph = ImportGraphTryBuilder {
            root_package: package_discovery::discover_package(root_package_path)?,
